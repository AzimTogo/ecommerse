  ...
  const handleTelegramAuthSuccess = async (telegramUser) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/v1/auth/telegram-register",
        {
          method: "POST",
          headers: {
      ...
      <div className="relative z-10 min-h-screen flex">
        <div className="hidden lg:flex lg:flex-1 bg-gradient-to-br from-indigo-600 via-purple-600 to-blue-700 relative overflow-hidden">
          <div className="absolute inset-0">
            ...
            <div
              className="absolute top-20 right-20 w-16 h-16 bg-yellow-400/30 rounded-lg rotate-45 animate-spin"
            ...
            <div
              className="absolute top-1/3 right-1/4 w-14 h-14 bg-orange-400/30 rounded-xl flex items-center justify-center animate-float backdrop-blur-sm"
              style={{ animationDelay: "-1s" }}
            >
              <i className="fas fa-gift text-xl text-white"></i>
            </div>
            <div
              className="absolute bottom-1/3 left-1/3 w-12 h-12 bg-pink-400/30 rounded-lg flex items-center justify-center animate-float backdrop-blur-sm"
